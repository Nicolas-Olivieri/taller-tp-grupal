...
void WithdrawItemCommand::build_snapshot(SnapshotBuilder& builder) {
    if (result.status == WithdrawItemStatus::PLAYER_UNBOUNDED) {
        const std::string& error_message = "Hablale a un banquero para poder retirar un ítem";
    ...
    static std::map<WithdrawItemStatus, std::string> result_to_message({
            ...
            {WithdrawItemStatus::ITEM_NOT_IN_BANK,
             "Estás tratando de retirar un ítem que no tenés en el banco"},
            {WithdrawItemStatus::INVENTORY_FULL,
             "Tu inventario ya está al máximo, deshazte de un tipo ítem primero"},
            {WithdrawItemStatus::GHOST_FAIL, "Perdón, no atiendo fantasmas"},
            {WithdrawItemStatus::ACTION_NOT_ACCEPTED, "Perdón, yo no puedo hacer eso"},
    });
