...
void WithdrawGoldCommand::build_snapshot(SnapshotBuilder& builder) {
    ...
    static std::map<WithdrawGoldStatus, std::string> result_to_message({
            ...
            {WithdrawGoldStatus::NOT_ENOUGH_GOLD,
             "Estás tratando de retirar mas oro del que tenes en el banco"},
            {WithdrawGoldStatus::INVENTORY_FULL, "Tu monedero está al máximo, gastá un poco de oro primero"},
            {WithdrawGoldStatus::GHOST_FAIL, "Perdón, no atiendo fantasmas"},
            {WithdrawGoldStatus::ACTION_NOT_ACCEPTED, "Perdón, yo no puedo hacer eso"},
    });
