...
void BuyCommand::build_snapshot(SnapshotBuilder& builder) {
    if (result.status == BuyStatus::PLAYER_UNBOUNDED) {
        const std::string& error_message = "Hablale a un sacerdote o comerciante para poder comprar un ítem";
    ...
    static std::map<BuyStatus, std::string> result_to_message(
            {{BuyStatus::ITEM_SOLD, "Gracias por tu compra"},
             ...
             {BuyStatus::NOT_ENOUGH_GOLD, "No te alcanza el oro para comprar ese ítem"},
             {BuyStatus::INVENTORY_FULL, "Tu inventario está al máximo, deshazte de un tipo ítem primero"},
             {BuyStatus::GHOST_FAIL, "Perdón, no le vendo a fantasmas"},
             {BuyStatus::ACTION_NOT_ACCEPTED, "Perdón, yo no puedo hacer eso"}});
