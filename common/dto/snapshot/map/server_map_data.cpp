...
ServerMapDataDTO::ServerMapDataDTO(const uint16_t w, const uint16_t h, const GridMatrixDTO& grid,
                                   const std::vector<AllyInfoDTO>& npcs, const std::vector<TeleportInfoDTO>& teleports):
...
size_t ServerMapDataDTO::message_size() const {
    ...
    return sizeof(width) + sizeof(height) + sizeof(uint16_t) * height * width + sizeof(uint16_t) + npcs_size + sizeof(uint16_t) + teleport_size;
}
