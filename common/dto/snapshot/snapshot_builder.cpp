...
PlayerInfoDTO SnapshotBuilder::convert_to_info(const std::string& player_name, const Player& player) {
    ...
    return PlayerInfoDTO(
            player_name, player.get_clan_name(), player.get_direction(), position.get_x(), position.get_y(),
            player.get_safe_gold(), player.get_excess_gold(), AppearanceDTO(body, player.get_head()),
...
LootInfoDTO SnapshotBuilder::convert_to_info(const std::pair<uint16_t, uint16_t>& position, Tile& tile) {
    return LootInfoDTO(tile.get_loot().top().type, position.first, position.second);
}
